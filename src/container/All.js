            ...
            <div>
                {
                    this.state.data.map((item)=>(
                        <div  className="word" style={{overflow:'hidden'}} key={item.id}>
                            <Link to={`/all/id`}><p ><img src={item.author.avatar_url}/>{item.reply_count}/{item.visit_count}
                            <button style={{width:45,height:25,'background-color':'grey'}}>精华</button>
                             <span style={{'font-weight':'bold','font-size':'15px'}}>{item.title}</span>
                             <span style={{float:'right'}}>3天前</span>
                            </p> </Link>
                            <Route path='/all/id' component={id}/>
                        </div>
                    ))
                }
                <div>
